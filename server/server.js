      ...
      return new Promise((resolve, reject) => {
        try {
          if (text.includes('DELETE')) {
            ...
            const info = stmt.run(...params);
            const row = sqliteDb.prepare('SELECT * FROM comments WHERE id = ?').get(info.lastInsertRowid);
            resolve({ rows: [row] });
...
app.post("/api/comments", checkJwt, async (req, res) => {
  try {
    ...
    if (!userPermissions.includes('write:comments')) {
      return res.status(403).json({ error: 'Missing write:comments permission' });
    }
...
app.delete("/api/comments/:id", checkJwt, async (req, res) => {
  try {
    ...
    if (!userPermissions.includes('delete:comments')) {
      return res.status(403).json({ error: 'Missing delete:comments permission' });
    }
